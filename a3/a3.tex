...
$\forall 1\leq i\leq n, y_i\left(w^T x_i\right) + y_i w_ij k \geq \delta$, 

where $w_ij$ = the weight coefficients for the corresponding $t_ij$. 

If $t_ij = 0$ $\foralli,j$, data is separable as given by the problem.
For $t_ij \neq 0$, let {\bf $w_ij} = {\large\frac{{\bf M y_i}}{{\bf k}}}$, and since $\mid {\bf x_i} \mid \leq {\bf M}$, the extra terms are always > 0 and guaranteed to be larger than terms generated by any missing attributes. So if $\delta$ is the lower bound for datasets with no missing features,
...
\subsection{c.}
So to keep the same linear classifier, the mistake bound, $\frac{R^2}{\delta^2}$ should be invariant to this scaling. Since $R = max\l x_i\l$ and x gets scaled by k, $R\prime = k*R$, and to keep the mistake bound invariant, $\delta\prime = k*\delta$. Since $\delta = \frac{1}{\lw\l}, w\prime = \frac{w}{k}$. Now since $\xi_i$ is in Ôunits of $\delta$Õ, $\xi_i$ should not change, so to have the same linear classifier C should become \frac{C}{k}.
